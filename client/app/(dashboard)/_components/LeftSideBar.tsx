...
const LeftSideBar = ({
  ...
  const { isLeftSheetOpen, setIsLeftSheetOpen } = useSheet();
  if (!groupData.success) {
    toast.error(groupData.error as string);
  }

  return (
    <div className="h-[800px] w-[15%] 2xl:block hidden">
      <div className="py-2">
      ...
      <Sheet open={isLeftSheetOpen} onOpenChange={setIsLeftSheetOpen}>
        <SheetContent side={"left"} className="w-[250px] p-0 pt-14">
          <SheetHeader className="flex flex-row justify-between  items-center">
            <h2 className="relative px-7 text-lg font-semibold tracking-tight flex items-center gap-2">
              <Group /> <span className="hidden 2xl:block">Groups</span>
            </h2>
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
